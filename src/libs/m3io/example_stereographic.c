/*
example_sterographic.c - Demonstrate usage of createStereographicM3IOFile().
cc -I. -o example_sterographic example_sterographic.c M3IO.c libnetcdf.a
./example_sterographic
ncdump example_sterographic.ncf
*/

#include <M3IO.h> /* For createPolarM3IOFile(). */


int main( void ) {
  const char* const fileName = "example_stereographic.ncf";
  enum {
  ...
  const double longitude0       = -98.0;
  const double latitude0        = 90.0;
  const double centralLongitude = -98.0;
  const double secantLatitude   = 90.0;
  ...
  if ( file != -1 ) {
    ...
    /* Demonstrate writing data per timestep: */

    for ( timestep = 0; ok && timestep < TIMESTEPS; ++timestep ) {
      const int offset = timestep * LAYERS * ROWS * COLUMNS;
      ok = writeM3IODataForTimestep( file, timestep, "HT", dataHt + offset );
    }
