/*
example_utm.c - Demonstrate createUTMM3IOFile().
cc -I. -o example_utm example_utm.c M3IO.c libnetcdf.a
./example_utm
ncdump example_utm.ncf
*/

#include <M3IO.h> /* For createUTMM3IOFile(). */


int main( void ) {
  const char* const fileName = "example_utm.ncf";
  enum {
    VARIABLES = 1, TIMESTEPS = 1, LAYERS = 1, ROWS = 7, COLUMNS = 8,
    YYYYDDD = 2009121
  };
  const double cellSize    = 5000.0;
  const double westEdge    = 350000.0;
  const double southEdge   = 3785000.0;
  const double longitude0  = 0.0;
  const double latitude0   = 0.0;
  const int zone           = 16;
  ...
  const Name gridName = { "NC_UTM_GRID" };
