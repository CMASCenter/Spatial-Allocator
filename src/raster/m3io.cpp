...
HISTORY: 2010/11/05 plessel.todd@epa.gov, Created.
         2011/11/15 Modified for Spatial Allocator, LR
******************************************************************************/
...
#define MISSIING_VALUE_SA     -999.0
...
/*By LR: added two new varaibles to functions:
     int firstTimestamp
     int startTime_att
  For writting global variables*/
...
static int writeTFLAG( int file, int variables, int timesteps,
                       int firstTimestamp, int startTime_att, int timeSteps_att );
...
static int createVariable( int file,
                           const char* name,
                           const char* units,
                           nc_type type,              //modifed by LR.  changed int to nc_type
                           int dimensionality,
...
INPUTS:  const char* fileName  Name of NetCDF file to create.
         ...
         int startTime_att     Starting time in the global attribute
         int timeSteps_att     Time step in the global attribute
         double xcellSize      Size in degree of grid cell width.
         double ycellSize      Size in degree of grid cell height.
...
int createLonLatM3IOFile( const char* fileName,
  ...
  assert( xcellSize < 90.0 );
  assert( ycellSize < 90.0 );
  ...
  return createM3IOFile( fileName, variables, timesteps, layers, rows, columns,
                         yyyyddd, startTime_att, timeSteps_att, xcellSize, ycellSize, westEdge, southEdge,
...
INPUTS:  const char* fileName  Name of NetCDF file to create.
         ...
         int startTime_att     Starting time in the global attribute
         int timeSteps_att     Time step in the global attribute
         double xcellSize      Size in meters of grid cell width.
         double ycellSize      Size in meters of grid cell height.
...
static int createM3IOFile( const char* fileName,
  ...
  assert( xcellSize > 0.0 );
  assert( ycellSize > 0.0 );
  ...
  if ( result != -1 ) {
    ...
    if ( ok ) {
      ...
      //modified by LR for handling no time step file
      ok = ok && writeTFLAG( result, variables, timesteps, yyyyddd, startTime_att, timeSteps_att );
    }
...
INPUTS:  int file              NetCDF file to write to.
         ...
         int timeSteps_att     Time step in the global attribute
         int projection        LAMGRD3 or POLGRD3.
         double xcellSize      Size in meters/degree of grid cell width.
         double ycellSize      Size in meters/degree of grid cell height.
         double westEdge       Distance from longitude0 to west  edge of grid.
         double southEdge      Distance from latitude0  to south edge of grid.
         double longitude0     Longitude that projects to 0.
         double latitude0      Latitude  that projects to 0.
...
static int writeAttributes( int file,
  ...
  assert ( startTime_att >= 0 );    //added by LR
  assert ( timeSteps_att >= 0 );    //added by LR
  assert( xcellSize > 0.0 );
  assert( ycellSize > 0.0 );
  assert( IS_VALID_PROJECTION( projection ) );
  assert( IN_RANGE( longitude0, -180.0, 180.0 ) );
  ...
  //modified by LR
  if ( startTime_att == 0 && timeSteps_att == 0 )
  {
     result = AND2( result, writeIntegerAttribute( file, "SDATE", startTime_att ) );
  }
  else
  {
     result = AND2( result, writeIntegerAttribute( file, "SDATE",firstTimestamp) );
  }
  result = AND2( result, writeIntegerAttribute( file, "STIME", startTime_att ) );   
  result = AND2( result, writeIntegerAttribute( file, "TSTEP", timeSteps_att ) );   
...
INPUTS:  int file           NetCDF file to write to.
         ...
         int timesteps      Number of timesteps.
         int firstTimestamp  First timestamp. E.g., 2010365.
         int startTime_att   Start time. E.g., 0
         int timeSteps_att   Time step. E.g. 240000
...
static int writeTFLAG( int file,
  ...
  assert( IN_RANGE( variables, 1, MXVARS3 ) );
  assert( isValidDate( firstTimestamp ) );    
  assert( startTime_att >= 0 );    //added by LR for no time step output
  assert ( timeSteps_att >= 0 );   //added by LR for no time step output
  ...
  if ( checkStatus( nc_inq_varid( file, "TFLAG", &id ),
    ...
    if ( data ) {
      ...
      const int tstep = 1;   //time step is one dayHHMMSS, LR
      ...
      do {
        ...
        do {
          ...
          //modified by LR for no time step 
          if ( startTime_att == 0 && timeSteps_att == 0 ) 
          {
             *dateTime++ = 0;
          }
          ...
          if ( startTime_att == 0 && timeSteps_att == 240000 )
          {
              *dateTime++ = 0;
          }
          else
...
static int createDimensions( int file,
  ...
  //added by LR 06/2011 for unlimited TSTEP
  int id = -1;
  
  if ( ! checkStatus( nc_def_dim( file, names[ index ], NC_UNLIMITED, &id ), "Can't create dimension" ) )
  {
      index = count;
  }
  else
...
static int createVariable( int file,
  ...
  if ( checkStatus( nc_def_var( file, name, type, dimensionality, dimensionIds, &id ),
    ...
    if ( type == NC_FLOAT )
    {
       if ( writeRealAttribute( file, id, NC_FLOAT, "missing_value", MISSIING_VALUE_IOAPI ) )
       {
         result = id;
       }
    }
