...
void readGOESImageFile ( string dayTimeStr, string imageFileName, float *goesV, string inGOESVarFileNameFmt );
void computeGOESVariable (int timeStep, int startMins, float *goesV, string inGOESVarFileNameFmt, 
...
int main(int nArgc, char *argv[])
{
	...
	*********************************/
        ...
        *******************************************/
        ...
        /*      Create Shapefile         */
        /*********************************/
        shapeFile = createGridShapes ( grid );  //created a temp Shapefile from grid definition
        grid.name = shapeFile;          //assign temp shapeFile to grid name for projection and rasterization usage

        /**********************************************/
        /*      Obtain image info from GOES dir       */
        /**********************************************/
        DIR         *dp,*dpFiles;         //image data directory
        struct      dirent *dirp, *dirpFiles;
        string      imageFile;
        ...
        //get the dir to get one day directory
        while ((dirp = readdir(dp)) != NULL)
        {
           tmp_str = string(dirp->d_name);
           tmp_str = trim(tmp_str);
           if ( tmp_str.find("gp_") !=string::npos )
           {
              dayDir = string (dataDir + tmp_str);
              dayDir = processDirName (dayDir);
       
              //open 1 day image data directory to get an Imager (4km) image file
              if((dpFiles  = opendir(dayDir.c_str())) != NULL)
              {
                 ...
                 {             
                    ...
                    tmp_str = trim(tmp_str);
                    if ( tmp_str.find(".grb") != string::npos && tmp_str.find(".snd.") == string::npos )
                    { 
                       i = tmp_str.length() - 4;  //.grb position
                       //printf ("lenght %s = %d\n", tmp_str.c_str(),i);
                       string bil_str = tmp_str.substr(i,4);
                       if ( bil_str.compare(".grb" ) == 0 )
                       {
                          imageFile = tmp_str;  //found the image for projection information
       ...
       if ( imageFile.empty() )
       {  
          printf ( "Error: no *.grb image file under directory: %s\n",dataDir.c_str() );
          exit ( 1 );
       }

       tmp_str =  string( dayDir );
       tmp_str.append( imageFile );  
       imageFile = tmp_str;      // use one of image files in preprocessed image dir
       ...
       /*********************************/
        gridInfo newRasterInfo = computeNewRasterInfo ( projectedSHPFile, rasterResolution, grid);

       /*********************************/
       /*    Rasterize  Shapefile       */
       /*********************************/
       gridRasterFile = toRasterFile (gdalBinDir, newRasterInfo, imageFile, projectedSHPFile, grid);
	...
	******************************/
	...
	anyErrors( nc_create(outNetcdfFile.c_str(), NC_CLOBBER, &ncid) );   //open output file
        ...
        *****************************************/
        ...
        {
           printf( "Compute GOES variable: %s   File format: %s*.*\n",inGOESVars[i].c_str(),inGOESVarFileNameFmts[i].c_str() );
           ...
           computeGOESVariable (timeStep,startMins,goesV,inGOESVarFileNameFmts[i],gridRasterFile,dataDir );
...
void setSelGOESVariables ( int index )
{
    ...
    if ( selection.compare("YES") == 0 )
    {
       ...
       printf( "\t\tGOES variable file format is: %s\n",tmp_str.c_str() );
...
string getdayStrFromFileName ( string imageFileName, string inGOESVarFileNameFmt )
{
     ...
     long      value; 
      ...
      {
         printf ("\t Time format should be YYYYMMDDhhZ.grb or YYYYMMDDhhmmZ.grb: %s\n",imageFileName.c_str() );
         exit ( 1 );
      }
...
void readGOESImageFile ( string dayTimeStr, string imageFileName, float *goesV, string inGOESVarFileNameFmt )
{
      ...
      ***********************/
      ...
      {
         printf( "\t\tError: Projection is not defined in satellite file: %s.\n", imageFileName.c_str() );
      ...
      /********************************************************
       *   If GOES image and grid domain image intersecting   *
       ********************************************************/
      ...
      { 
        ...
        for( i = 0; i<yCells_grd; i++ )
        {
           ...
           //get cell center point y and compute row in GOES image
           y = yMax_grd - i * yCellSize_grd - yCellSize_grd / 2.0;
           row = (int) (floor ((ymax - y) / yCellSize));   //start from 0 UL
           if ( row >= 0 && row < yCells )
           {
              for (j=0; j<xCells_grd; j++)
              {
                 ...
                 {
                    //get cell center point x and compute col in GOES image
                    x = xMin_grd + j * xCellSize_grd + xCellSize_grd / 2.0;
                    col = (int) (floor ((x - xmin) / xCellSize));  //start from 0 UL
                    if ( col >=0 && col < xCells )
                    {
                       pixelIndex = xCells * row + col;
                       value = poImage[pixelIndex];     //get GOES image value
                       if ( value != GOES_MISSIING_VALUE && value != 9999.0 )
                       {
                          gridIDs[gridID - 1] += 1;             //count GOES cells                          
                          gridValues[gridID - 1] += value;      //sum the GOES value for a domain grid
                       }
                    }  //col
                 }  //check gridID
              }  //j
     ...
     {
        printf ("\t\tGOES image does not intersect with domain grid image.\n");
    ...
    { 
       ...
       if((dpFiles  = opendir( dayDirName.c_str()) ) != NULL)
       {
          ...
          while ((dirpFiles = readdir(dpFiles)) != NULL)
          {
             tmp_str = string(dirpFiles->d_name);
             tmp_str = trim(tmp_str);
             if ( tmp_str.find( inGOESVarFileNameFmt.c_str() )!=string::npos && tmp_str.find( ".grb" )!=string::npos )
             {
       ...
       for ( i=time1; i<=time2; i++ )
       {
          ...
          {
             readGOESImageFile ( dayTimeStr, it_fileNameList->second.c_str(), goesV, inGOESVarFileNameFmt );
          }
          else
          {
             fillGOESMissingValues ( dayTimeStr, goesV );
