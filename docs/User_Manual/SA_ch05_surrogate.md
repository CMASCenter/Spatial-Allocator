...
There are many variables within the scripts/PG commands. This section documents (a) the variable name in the scripts, (b) description of the variable, (c) if variable is for all scripts or a list of scripts, and (d) how the value will be provided.

NOTE: PostgreSQL wants all database/schema/table/field names to be lower case. The C Surrogate Tools wants at least the field names to be in UPPER CASE. Therefore, when both the C and PG Surrogate Tools use the same field, it must be passed in the proper case to each tool.

Some of the variables are available from the surrogate_specifications.csv file (SScsv). Additional variables are required to handle the needs of a database; these include schema name, table name, field name, and spatial reference ID. There are several proposed changes to the input csv files that are read by the existing Surrogate Tool/Spatial Allocator. Most of the new fields will be placed at the end of the current records. Any changes that may be required to the existing programs are expected to be minimal.
